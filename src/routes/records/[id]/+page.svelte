<script lang="ts">
	...
	import type { DiagnosticRecord, UserAnalysisResult } from '$lib/types';
		...
		{#if record}
			...
			<div class="glass-card p-7 sm:p-9 mb-5">
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pb-5 border-b border-rose-100/80 stagger">
					<div>
						...
						<p class="font-semibold text-ink">{record.first_name}</p>
					</div>
					<div>
					...
					<div>
						...
						<p class="font-medium text-ink text-sm">{formatDateLong(record.created_at)}</p>
					</div>
				</div>
				...
				<!-- <div class="pt-6 space-y-5 stagger">
					<div>
						...
						{#if record.status === 'Positive'}
							<div class="flex items-center gap-2.5">
								...
								<span class="font-display italic text-xl font-semibold text-rose-700">{analysisResult?.analysis.status}</span
								>
							</div>
						{:else}
							<div class="flex items-center gap-2.5">
								<span class="w-2.5 h-2.5 rounded-full bg-emerald-500"></span>
								<span class="font-display italic text-xl font-semibold text-emerald-700"
									>{analysisResult?.analysis.status}</span
								>
							</div>
						{/if}
					</div>
					<div class="grid grid-cols-2 gap-5">
						<div>
							...
							<p class="font-semibold text-ink">{analysisResult?.analysis.grade}</p>
						</div>
						<div>
							<p class="field-label">Film Type</p>
							 <p class="font-semibold text-ink">{analysisResult?.analysis.filmType}</p> 
						</div>
					</div>
				</div> -->
			</div>
			...
			<div class="glass-card p-7 sm:p-9">
				<div class="flex items-center justify-between mb-5">
					...
					<span class="text-xs text-ink-soft font-medium">{analysisResult?.count} captured</span>
				</div>

				<div class="grid grid-cols-2 sm:grid-cols-3 gap-3 stagger">
					{#each analysisResult?.analysis ?? [] as snap, i (i)}
						<button
							type="button"
						...
						>
							...
							></div>
							<span
								class="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-black/40 backdrop-blur text-white text-[10px] font-mono"
								>#{i + 1}</span
							>
						</button>
					{/each}
